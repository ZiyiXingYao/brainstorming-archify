 ...
 * （流程名或章节语义名）按固定算法派生，供 `brainstorming` 在落盘时调用。
 *
 * 算法（design.md 决策四 / execution-contract 接口约束 3）
 * ------------------------------------------------------
 *   1. 保留字母（含 CJK）、数字；其余字符（空白、标点、路径分隔符等）替换为 `-`；
 *   2. 连续 `-` 折叠为单个 `-`；
 *   3. 去掉首尾 `-`；
 *   4. ASCII 部分小写化（非 ASCII 原样保留——`toLowerCase()` 会顺带改动非 ASCII，
 *      故只对 `[A-Z]` 逐字符降格）；
 *   5. 派生结果为空时回落到该图的图类型名（如 `architecture`）；
 *   6. 派生结果冲突时追加 `-2`、`-3`（依次递增），并把冲突双方报告出来，不静默改名。
 *
 * 「字母」用 Unicode 属性 `\p{L}` 表达，天然覆盖 CJK；「数字」用 `\p{N}`。
 *
 * 本文件为**新增文件**，不修改任何上游搬运文件，也不安装任何依赖。
 */

/** 允许保留的字符类：字母（含 CJK）与数字。 */
