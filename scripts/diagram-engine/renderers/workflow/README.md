...
This diagram-engine build ships `render` / `validate` / `doctor` only, so the
upstream `migrate` command is **not available** here. Author new workflows
directly on schema v2. When an existing v1 source must keep its fixed legacy
geometry, keep it on v1 as-is: do not change only `schema_version` on a document
that carries absolute coordinates, because v2 derives geometry from measured
content and the pinned coordinates would no longer describe the layout.

Inspect the stable author-facing v2 plan with:

```bash
node bin/render-driver.mjs validate workflow input.workflow.json --layout-json
```
