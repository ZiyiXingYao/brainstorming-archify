...
export function writeDiagram({ outPath, template, diagramType, meta, svg, cards }) {
  ...
  fs.writeFileSync(outPath, applyTemplate(template, {
    ...
    // Source evidence was removed together with the --repo-root capability.
    // Pass null so the template's evidence placeholder is still blanked out
    // instead of leaking a literal marker into the generated HTML.
    sourceEvidence: null,
  }));
