#!/usr/bin/env node
/**
 * render.mjs —— 绘图内核的唯一入口
 * ============================================================================
 *
 * 用法
 * ----
 *   render   <type> <input.json> <outdir>   渲染并一次性产出三件套
 *   validate <type> <input.json> [...]      只校验，不写任何产物（透传给内部驱动）
 *   doctor                                  环境与内核完整性自检（透传给内部驱动）
 *
 * 三件套（同一 outdir、同一前缀）
 * ------------------------------
 *   <basename>.json   源 IR 逐字节落盘
 *   <basename>.svg    自包含静态矢量图：从渲染结果 HTML 中切出 <svg>，
 *                     内联字体、样式规则、深/浅两套主题变量与背景
 *   <basename>.html   完整交互式页面，保留全部查看器交互能力
 *
 *   basename 取自输入 IR 的文件名（去掉扩展名）。设计文档场景下 IR 以
 *   `<文档序号>-<图名>.<图类型>.json` 命名，三件套因此同前缀同目录。
 *
 * 为什么按「第一个 <svg> 到第一个 </svg>」切片
 * --------------------------------------------
 * 渲染产物 HTML 里 `<svg` 与 `</svg>` 各恰好出现 1 次（已实测），切片确定可靠；
 ...
 * 目标同目录的 staging 目录，全部就绪后再 rename 提交；提交阶段任一步失败，
 * 已提交的步骤会回滚、既有文件被还原。
 *
 * 校验硬门与两轮降级
 * ------------------
 * 校验不过即非 0 退出且磁盘无产物（硬门）。其上再叠一层「两轮降级」：
 ...
 *
 * 渲染与校验的实现体是内部驱动 bin/render-driver.mjs（只含 render/validate/doctor
 * 三个子命令）；本文件是唯一对外入口，不再有第二个入口。
 */
...
const DRIVER = path.join(SKILL_ROOT, 'bin', 'render-driver.mjs');
...
/**
 * 由「输出目录 + 输入 IR 路径」派生三件套的目标路径。
 *
 * basename 取自输入 IR 的文件名（去掉扩展名），三个目标同目录、同前缀、
 * 扩展名不同。设计文档场景下 IR 命名为 `<文档序号>-<图名>.<图类型>.json`，
 * 于是三件套自然是 `<文档序号>-<图名>.<图类型>.{json,svg,html}`。
 *
 * @param {string} outDir
 * @param {string} inputPath
 * @returns {{outJson: string, outSvg: string, outHtml: string}}
 */
export function deriveArtifactPaths(outDir, inputPath) {
  const ext = path.extname(inputPath);
  const base = ext ? path.basename(inputPath, ext) : path.basename(inputPath);
  return {
    outJson: path.join(outDir, `${base}.json`),
    outSvg: path.join(outDir, `${base}.svg`),
    outHtml: path.join(outDir, `${base}.html`),
  };
}
...
/**
 * 多文件原子提交：先备份既有目标，再 rename 候选进来；任一步失败则整体回滚。
 * @param {{target: string, candidate: string}[]} entries
 */
...
function trackFailureRound({ type, input, outSvg, validate }) {
  ...
  if (!receipt) {
    process.stderr.write('render: 校验未产出可解析的结构化诊断，跳过两轮降级判定。\n');
    return null;
  }
  ...
  try {
    ...
    process.stderr.write(`render: 读取轮次状态失败（${error.message}），本轮按第 1 轮处理。\n`);
    history = [];
  }
  ...
  try {
    ...
    process.stderr.write(`render: 记录轮次状态失败（${error.message}），两轮降级判定可能失真。\n`);
  }
...
/**
 * `render` 子命令主流程，返回进程退出码（0 = 成功）。
 *
 * 一次性产出三件套：`<basename>.json`（源 IR 逐字节）、`<basename>.svg`（自包含
 * 静态图）、`<basename>.html`（完整交互式页面）。basename 取自输入 IR 的文件名。
 *
 * 注意：函数体内一律 `return` 退出码而不调用 `process.exit`——`process.exit`
 * 会立刻终止进程、**跳过 finally**，导致临时目录泄漏。
 *
 * @param {string[]} args
 * @returns {number}
 */
function commandRender(args) {
  if (args.length !== 3) {
    console.error('用法：render <type> <input.json> <outdir>');
    return 2;
  }
  const [type, inputArg, outDirArg] = args;
  ...
  const { outJson, outSvg, outHtml } = deriveArtifactPaths(outDir, input);
  ...
  if (validate.status !== 0) {
    ...
    if (tracked) {
      ...
      process.stderr.write(
        `render: validation failed (round ${tracked.decision.round}, target errors ${tracked.decision.currentErrors}`
  ...
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'render-html-'));
  const tmpHtml = path.join(tmpDir, `${type}.html`);
  try {
    const render = runNode([DRIVER, 'render', type, input, tmpHtml]);
    if (render.status !== 0) {
    ...
    // 3) 目标同目录 staging + 三件套原子提交（校验/渲染/组装都过了才碰目标）。
    fs.mkdirSync(outDir, { recursive: true });
    const stagingDir = fs.mkdtempSync(path.join(outDir, '.render-staging-'));
    try {
      const svgCandidate = path.join(stagingDir, 'candidate.svg');
      const htmlCandidate = path.join(stagingDir, 'candidate.html');
      ...
      commitArtifacts([
        ...
        { target: outHtml, candidate: htmlCandidate },
...
function usage() {
  return `Usage:
  render   <type> <input.json> <outdir>
  validate <type> <input.json> [--json] [--layout-json] [--quality standard|showcase]
  doctor

Types:
  architecture, workflow, sequence, dataflow, lifecycle

render writes three same-prefix files into <outdir>:
  <basename>.json  /  <basename>.svg  /  <basename>.html
(<basename> is the input IR file name without its extension)
`;
}
